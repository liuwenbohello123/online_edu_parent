...
public class TPermissionController {
    ...
    @ApiOperation(value = "1.回显角色拥有的权限")
    @GetMapping("queryAssignedPermission/{roleId}")
    public RetVal queryAssignedPermission(@PathVariable String roleId){

       List<TPermission> permissionList= permissionService.queryAssignedPermission(roleId);
        return RetVal.success().data("permissionInfo",permissionList);
    }

    @ApiOperation(value = "2.修改角色拥有的权限")//第一个参数 角色id 已经分配的权限id集合
